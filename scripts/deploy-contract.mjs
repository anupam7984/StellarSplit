...
const { Keypair, Networks, Server, TransactionBuilder, Account, xdr } = pkg;
...
async function deployContract() {
  ...
  const wasmHash = await rpcServer.uploadWasm(wasmBuffer, keypair, Networks.TESTNET);
  console.log('WASM uploaded. Hash:', wasmHash);

  const contractId = await rpcServer.createContract(keypair, Networks.TESTNET, wasmHash);
